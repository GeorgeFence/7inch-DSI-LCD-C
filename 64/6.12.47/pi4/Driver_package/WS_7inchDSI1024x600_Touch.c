 ...
 *
 * Updated for Linux kernel 6.12.47+rpt-rpi-v8 (aarch64)
 *
...
/* ---------------------------------------------------------------------------
 * Config checksum helpers
 * (same algorithm used by WS_calc_cfg_checksum_8 / _16 in the 5.15 binary)
 * ---------------------------------------------------------------------------
 */

/*
 * ws_calc_cfg_checksum_8 – 8-bit checksum of the config block.
 * Returns the one's complement of the byte sum, mod 256.
 */
static u8 ws_calc_cfg_checksum_8(const u8 *config, size_t len)
{
	u8 sum = 0;
	size_t i;
...
/*
 * ws_check_cfg_8 – verify 8-bit config checksum.
 * Returns 0 if valid, -EINVAL otherwise.
 */
static int ws_check_cfg_8(const u8 *config, size_t len)
{
	u8 checksum = ws_calc_cfg_checksum_8(config, len - 1);

	if (checksum != config[len - 1]) {
		return -EINVAL;
	}
	return 0;
}
...
/*
 * ws_calc_cfg_checksum_16 – 16-bit checksum of the config block.
 * Returns the two's complement of the sum, mod 65536.
 */
static u16 ws_calc_cfg_checksum_16(const u8 *config, size_t len)
{
	u16 sum = 0;
	size_t i;

	for (i = 0; i < len; i += 2) {
		u16 word = ((u16)config[i] << 8) | config[i + 1];
...
/*
 * ws_check_cfg_16 – verify 16-bit config checksum.
 * Returns 0 if valid, -EINVAL otherwise.
 */
static int ws_check_cfg_16(const u8 *config, size_t len)
{
	u16 checksum;
	u16 stored;
	...
	checksum = ws_calc_cfg_checksum_16(config, len - 2);
	stored   = ((u16)config[len - 2] << 8) | config[len - 1];
